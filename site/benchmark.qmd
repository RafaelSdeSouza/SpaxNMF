...
The saved figure emphasizes where the model-selected sparse-component peaks land
on an asinh-stretched white-light image, how the corresponding sparse-component
maps look, and how the reconstruction compares at those peak locations.
...
- The white-light panel is the key visual check: if the model is isolating GC-like sources, the sparse-component peaks should land on visually compact knots in the asinh image.
- The sparse-component maps panel is the main spatial diagnostic for whether the sparse prior is isolating localized structure.
...
Even in the saved run above, the sparse components should be read cautiously:
the current model improves concentration metrics relative to vanilla NMF, but
the candidate peaks still collapse onto the bright central galaxy light rather
than separating visually distinct GC-like sources. At this stage the benchmark
is a useful failure case, not a polished detection result.
...
component-weight maps. On the current NGC1512 run, the answer is only partial:
the mixed prior improves concentration and reconstruction relative to vanilla
NMF, but it does not yet produce a convincing GC-isolation result.
